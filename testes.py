...
class alunoTest(unittest.TestCase):
  ...
  def test_salvarAluno(self):   
    ...
    self.assertEqual(True, resposta, 'Aluno nao foi persistido corretamente!');
    documento = self.conexao['aluno'].find_one({'nome': self.aluno.nome, 'sobrenome': self.aluno.sobrenome});
    self.assertIsNotNone(documento, 'Documento nao encontrado na colecao!');
    self.assertEqual(self.aluno.nota, documento['nota'], 'Nota do aluno persistida incorretamente!');
